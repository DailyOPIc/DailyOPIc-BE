...
class FakeResponses:
    ...
    async def create(self, **kwargs: object) -> SimpleNamespace:
        ...
        requested_slots = set(schema["properties"])  # type: ignore[index]
        payload = {
            f"q{question.number:02d}": {
                "prompt": question.prompt,
                "followUpPrompt": question.follow_up_prompt,
                "rubricFocus": question.rubric_focus,
            }
            for question in questions
            if f"q{question.number:02d}" in requested_slots
        }
...
def mock_evaluation_json(rubric_count: int = 5) -> str:
    payload = {
        ...
        "rubrics": [
            {
                ...
                "band": RubricBand.FUNCTIONAL.value,
                "evidence": "답변에서 핵심 의미와 근거가 확인됩니다.",
                "nextAction": "구체적인 사례를 한 단계 더 연결하세요.",
            }
            for dimension in list(RubricDimension)[:rubric_count]
        ],
...
def test_openai_strict_schema_requires_nullable_question_fields() -> None:
    schema = openai_strict_json_schema(
        AIService._question_content_schema(
            practice_questions("schema")[:2]
        ).model_json_schema()
    )
    question_schema = schema["$defs"]["GeneratedQuestionContent"]
    ...
    assert "questionStyle" not in serialized
    assert "examSection" not in serialized
...
@pytest.mark.asyncio
async def test_mock_evaluation_retries_when_rubric_validation_fails() -> None:
    repository = QuestionPatternRepository(Path("app/data/question_patterns.json"))
    service = AIService(api_key="test-key", model="test-model", mock=False, repository=repository)
    questions = [
    ...
    service._client = FakeTextOpenAIClient(  # type: ignore[assignment]
        [mock_evaluation_json(rubric_count=4), mock_evaluation_json()]
    )
    ...
    assert [item.dimension for item in result.rubrics] == list(RubricDimension)
    ...
    assert "rubrics must contain all five dimensions in order" in str(
        retry_request["instructions"]
    )
...
async def test_daily_pool_rewrites_recent_topic_id_collisions() -> None:
    ...
    assert result.questions[0].topic_id.startswith(f"{generated[0].topic_id}_")
...
async def test_generation_retry_includes_previous_validation_errors() -> None:
    ...
    assert {item["questionNumber"] for item in input_text["previousValidationErrors"]} == {2, 3}
    assert set(item["number"] for item in input_text["blueprint"]) == {2, 3}
    assert "requested failed slots" in input_text["retryInstructions"][0]


@pytest.mark.asyncio
async def test_real_ai_retries_only_prompt_that_reuses_recent_content() -> None:
    repository = QuestionPatternRepository(Path("app/data/question_patterns.json"))
    service = AIService(api_key="test-key", model="test-model", mock=False, repository=repository)
    duplicate = practice_questions("duplicate")
    fresh = practice_questions("fresh")
    ...
    assert service._client.responses.calls == 2  # type: ignore[union-attr]
    second_request = service._client.responses.requests[1]  # type: ignore[union-attr]
    second_input = json.loads(str(second_request["input"]))
    assert [item["number"] for item in second_input["blueprint"]] == [1]
    assert result.questions[0].prompt == fresh[0].prompt
    assert result.questions[1].prompt == duplicate[1].prompt


@pytest.mark.asyncio
async def test_real_ai_uses_validated_fallback_after_retry_exhaustion() -> None:
    repository = QuestionPatternRepository(Path("app/data/question_patterns.json"))
    service = AIService(api_key="test-key", model="test-model", mock=False, repository=repository)
    duplicate = practice_questions("duplicate")
    duplicate[1] = duplicate[1].model_copy(
    ...
    service._client = FakeOpenAIClient([duplicate, duplicate, duplicate])  # type: ignore[assignment]
    history = {"setHashes": [], "topicIds": [], "promptHashes": []}

    result = await service.generate_practice(
        OPIcLevel.IH, BackgroundProfile(), history=history
    )

    assert result.fallback_used is True
    assert set(result.fallback_question_numbers) == {2, 3}
...
async def test_real_ai_rejects_recent_prompt_hash() -> None:
    ...
    assert result.fallback_used is True
    assert result.fallback_question_numbers == (1,)
...
async def test_real_ai_rejects_recent_set_hash() -> None:
    ...
    duplicate = practice_questions("duplicate")
    service._client = FakeOpenAIClient([duplicate, duplicate, duplicate])  # type: ignore[assignment]
    blueprint = FallbackQuestionGenerator(repository).practice_front(
        5, BackgroundProfile()
    )
    expected = [
        base.model_copy(
            update={
                ...
                "follow_up_prompt": creative.follow_up_prompt,
                "rubric_focus": creative.rubric_focus,
            }
        )
        for base, creative in zip(blueprint, duplicate)
    ]
    serialized = [item.model_dump(by_alias=True, mode="json") for item in expected]
    history = {
