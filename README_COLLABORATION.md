...
로컬 개발과 운영은 같은 구조를 사용하며, AI 호출만 `MOCK_AI`로 전환합니다.
...
개발 서버도 Firestore와 Firebase App Check를 사용합니다. iOS 시뮬레이터는 App Check debug token을 Firebase Console에 등록해 테스트합니다.
...
서버는 필수 외부 설정이 없거나 `MOCK_AI=false`인데 `OPENAI_API_KEY`가 없으면 시작 단계에서 실패해야 합니다.
...
- [ ] App Check debug/production token 검증 확인
